---
name: code-review
description: Adversarial senior-maintainer review of a pull request — decides whether the change deserves to be merged
---
...
This repository is `brainrot-webpage` — the docs site and in-browser
playground for the Brainrot language. See `AGENTS.md` for architecture,
commands, and the invariants the playground depends on.

Apply the standard above to the constraints that actually exist here:

* **Module reuse.** The Brainrot interpreter holds global state. Any change
  that reuses an Emscripten module instance, or a Worker, across runs is a
  correctness defect regardless of how much faster it looks.
* **Timeout ownership.** The load budget (`LOAD_TIMEOUT_MS`) and the
  program budget (`timeoutMs`) are distinct, and only the main thread can
  terminate a Worker. A patch that merges the two budgets, or moves the
  watchdog into the Worker, is BLOCKING — trace it, do not assume.
* **Error taxonomy.** `RuntimeLoadError` means the runtime never came up;
  a plain `Error` means the program crashed after loading. Callers branch on
  this. Verify a change preserves the distinction on every path, including
  `onerror`.
* **Version pinning.** `src/wasmVersion.json` is the single source of truth
  and feeds the `?v=` cache-buster. A second copy of that version, anywhere,
  is duplicated machinery. So is any second implementation of
  `runInModule`'s logic that is not the deliberate, documented plain-JS one
  in `scripts/verify-wasm-runtime.mjs`.
* **Caching contract.** `index.html` must be `no-cache`; `*.mjs` must be
  served as `application/javascript`. That rule exists in both
  `.github/workflows/deploy.yml` and `nginx.conf`. A change to one and not
  the other is a real, shippable bug.
* **Test layering.** `runtime.test.ts` mocks the Worker and proves nothing
  about wasm interop; `scripts/verify-wasm-runtime.mjs` runs the real
  artifact but cannot exercise termination. If a PR adds wasm-facing
  behavior covered only by mocked tests, say so plainly: CI is green, and
  the contract is untested.
* **Comment accuracy.** This codebase's comments carry load-bearing
  rationale. A behavior change that leaves a comment describing the old
  behavior is a bug, not a nitpick.
